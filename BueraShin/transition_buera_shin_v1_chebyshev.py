...
def plot_policy_2d(coeffs, iteration, name="post", outdir="plots", coeffs_alt=None):
    """
    Generates a 2D plot of the policy function.
    Case 1 (Post-Reform): Plots Median z and Max z.
    Case 2 (Pre-Reform): Plots Max z (distorted), Max z (undistorted), and Median z.
    """
    if not os.path.exists(outdir):
        os.makedirs(outdir, exist_ok=True)
        
    limit_a = 200.0
    nodes_a, nodes_z, _ = generate_bivariate_nodes_matrix(A_MIN, A_MAX, Z_MIN, Z_MAX)
    a_dense = np.linspace(A_MIN, limit_a, 500)
    
    idx_max = len(nodes_z) - 1
    idx_med = len(nodes_z) // 2
    z_max = nodes_z[idx_max]
    z_med = nodes_z[idx_med]

    fig, ax = plt.subplots(figsize=(10, 7))
    ax.plot([0, limit_a], [0, limit_a], 'k--', alpha=0.3, label='45-degree')
    
    # helper for plotting a single line
    def plot_line(c, z, label, color, style, marker_color):
        ...
        mask = nodes_a <= limit_a
        ax.scatter(nodes_a[mask], pol_nodes[mask], color=marker_color, s=25, alpha=0.6, edgecolors='k', zorder=5)

    if coeffs_alt is not None:
        # Pre-reform case: coeffs=plus, coeffs_alt=minus
        plot_line(coeffs, z_max, f"Max z: Distorted ($\\tau^+$)", '#D62728', '-', '#D62728')
        plot_line(coeffs_alt, z_max, f"Max z: Subsidized ($\\tau^-$)", '#2CA02C', '-', '#2CA02C')
    ...
    ax.set_title(f"Spectral Policy Approximation: {name} (Iteration {iteration})")
    ...
    save_path = os.path.join(outdir, f"policy_2d_{name}.png")
    plt.savefig(save_path, dpi=120)
    plt.close()

def plot_grid_comparison(outdir="plots"):
    """
    Plots the two asset grids in a single, fancy diagnostic plot.
    """
    if not os.path.exists(outdir):
        os.makedirs(outdir, exist_ok=True)
        
    # 1. Sparse Chebyshev Nodes
    nodes_a, _, _ = generate_bivariate_nodes_matrix(A_MIN, A_MAX, Z_MIN, Z_MAX)
    
    # 2. Dense Distribution Grid
    na_h = 600
    a_h = np.exp(np.linspace(np.log(A_MIN+A_SHIFT), np.log(A_MAX+A_SHIFT), na_h)) - A_SHIFT
    
    # Selection for zoom visibility
    zoom_limit = min(100.0, A_MAX)
    
    plt.style.use('seaborn-v0_8-whitegrid')
    fig, ax = plt.subplots(figsize=(12, 5))
    
    # Use rich colors
    color_cheb = '#E63946' # Imperial Red
    color_mass = '#457B9D' # Celadon Blue
    
    # Plot Dense Grid as a light background "carpet"
    ax.vlines(a_h[a_h <= zoom_limit], 0, 0.4, color=color_mass, alpha=0.3, linewidth=0.5, label='Mass Grid (Dense)')
    
    # Plot Chebyshev Nodes as prominent markers
    ax.vlines(nodes_a[nodes_a <= zoom_limit], 0, 1.0, color=color_cheb, alpha=0.9, linewidth=2.5, label='Chebyshev Nodes (Sparse)')
    
    # Add dots on top for extra "pop"
    ax.scatter(nodes_a[nodes_a <= zoom_limit], np.ones_like(nodes_a[nodes_a <= zoom_limit]), 
               color=color_cheb, s=40, zorder=5, edgecolors='white', linewidth=1)

    # Styling
    ax.set_title("Multi-Grid Architecture: Policy vs. Distribution", fontsize=15, fontweight='bold', pad=20)
    ax.set_xlabel("Asset Value $a$", fontsize=12)
    ax.set_yticks([]) # Hide Y axis
    ax.set_xlim(-1, zoom_limit + 1)
    ax.set_ylim(-0.1, 1.25)
    
    # Legend with "glass" effect styling if possible, but keep it simple
    ax.legend(loc='upper right', frameon=True, facecolor='white', framealpha=0.8, fontsize=11)
    
    # Descriptive text
    ax.text(0.01, 0.9, "Policy uses local polynomial\ninterpolation on sparse nodes", 
            transform=ax.transAxes, fontsize=10, color=color_cheb, fontweight='bold')
    ax.text(0.01, 0.35, "Distribution uses histogram\nmass on a dense grid", 
            transform=ax.transAxes, fontsize=10, color=color_mass, fontweight='bold')

    plt.tight_layout()
    save_path = os.path.join(outdir, "asset_grid_comparison.png")
    plt.savefig(save_path, dpi=200) # Higher DPI for "fancy" look
    plt.close()
    print(f"Fancy grid comparison plot saved to {save_path}")
...
def solve_policy_spectral(params, w, r, coeffs_init=None):
    ...
    M_vals = np.concatenate([np.linspace(0.0, 0.998, 38), [0.999, 0.9995]])
    ...
    for it in range(150):
        ...
        # Plot 2D while iterating
        if it % 20 == 0:
            plot_policy_2d(coeffs, it, name="post_reform")
            
        if diff < 1e-7: 
            plot_policy_2d(coeffs, it, name="post_reform") # Final converged plot
            break
    return coeffs
...
def solve_policy_spectral_with_dist(params, w, r, tau_plus, tau_minus, q_dist,
    ...
    for it in range(150):
        ...
        # Plot 2D while iterating (tau_plus)
        if it % 20 == 0:
            plot_policy_2d(coeffs_plus, it, name="pre_reform_distorted", coeffs_alt=coeffs_minus)

        if diff < 1e-7:
            plot_policy_2d(coeffs_plus, it, name="pre_reform_distorted", coeffs_alt=coeffs_minus) # Final
            break
...
def plot_policy_comparison(pre_eq, post_eq, output_dir='outputs'):
    """
    Plots policy functions for different ability levels to compare 
    pre-reform (distorted) and post-reform (undistorted) states.
    """
    os.makedirs(output_dir, exist_ok=True)
    
    z_grid = pre_eq['z_grid']
    
    # Dense asset grid for smooth plotting
    # Focus on the relevant range where constraints might bind
    limit_a = 200.0
    a_dense = np.linspace(A_MIN, limit_a, 500)
    
    # Select ability indices to show (Low, Median, High)
    indices = [0, len(z_grid)//2, len(z_grid)-1]
    z_vals = [z_grid[i] for i in indices]
    colors = ['#1f77b4', '#ff7f0e', '#2ca02c'] # Blue, Orange, Green
    labels = [f'Low z ({z_vals[0]:.2f})', f'Med z ({z_vals[1]:.2f})', f'High z ({z_vals[2]:.2f})']

    fig, axes = plt.subplots(1, 2, figsize=(14, 6), sharey=True)

    # --- Pre-Reform (Distorted) ---
    ax = axes[0]
    for i, idx in enumerate(indices):
        z_val = z_grid[idx]
        # Plot the policy for tau_plus (more likely to be binding/distorted)
        pol_p = bivariate_eval_matrix(a_dense, np.array([z_val]), pre_eq['coeffs_plus'], 
                                     A_MIN, A_MAX, Z_MIN, Z_MAX)
        ax.plot(a_dense, pol_p.flatten(), color=colors[i], label=labels[i])
        
    ax.plot(a_dense, a_dense, 'k--', alpha=0.3, label='45-degree')
    ax.set_title("Pre-Reform Policy ($\tau^+$ type)", fontsize=14)
    ax.set_xlabel("Current Assets $a$")
    ax.set_ylabel("Next Period Assets $a'$")
    ax.set_xlim(0, limit_a)
    ax.set_ylim(0, limit_a)
    ...
    # --- Post-Reform (Standard) ---
    ax = axes[1]
    for i, idx in enumerate(indices):
        z_val = z_grid[idx]
        pol = bivariate_eval_matrix(a_dense, np.array([z_val]), post_eq['coeffs'], 
                                   A_MIN, A_MAX, Z_MIN, Z_MAX)
        ax.plot(a_dense, pol.flatten(), color=colors[i], label=labels[i])
        
    ax.plot(a_dense, a_dense, 'k--', alpha=0.3)
    ax.set_title("Post-Reform Policy (No Distortion)", fontsize=14)
    ax.set_xlabel("Current Assets $a$")
    ax.set_xlim(0, limit_a)
    ax.legend(frameon=False, fontsize=10)
    ax.grid(True, alpha=0.2)
    ...
    plt.tight_layout()
    save_path = os.path.join(output_dir, 'policy_comparison.png')
    plt.savefig(save_path, dpi=150)
    plt.close()
    print(f"Policy plots saved to {save_path}")
...
def main():
    ...
    # Diagnostic: Plot Grids at start
    plot_grid_comparison()
    print(f"All diagnostic plots (grid, 3D policy) will be saved in the 'plots/' directory.")
    ...
    plot_policy_comparison(pre_eq, post_eq, args.output_dir)
