...
- Pre-computed transition matrices for ability process
- Efficient sparse matrix for stationary distribution
...
print("=" * 60)
print("Buera-Shin (2010) Replication - V2 OPTIMIZED")
...
# =============================================================================
# Entrepreneur Problem - Vectorized for speed
# =============================================================================
...
# =============================================================================
# Value Function Iteration - Parallelized
# =============================================================================
    ...
    for i_z in prange(n_z):
        ...
        for i_a in range(n_a):
            ...
            for i_a_prime in range(n_a):
                ...
                val = utility(c, sigma) + beta * EV[i_a_prime, i_z]
                if val > best_val:
                    best_val = val
                    best_idx = i_a_prime
...
# =============================================================================
# Stationary Distribution - Sparse matrix method
# =============================================================================

def compute_stationary_distribution(policy_a_idx, a_grid, z_grid, prob_z, psi):
    """Compute stationary distribution using sparse eigenvector method"""
    n_a, n_z = len(a_grid), len(z_grid)
    n_states = n_a * n_z
...
def compute_aggregates_on_grid(a_grid, z_grid, policy_occ, w, r, lam, delta, alpha, upsilon):
    ...
    for i_a in range(n_a):
        for i_z in range(n_z):
            if policy_occ[i_a, i_z] > 0.5:
                a, z = a_grid[i_a], z_grid[i_z]
                profit, kstar, lstar = solve_entrepreneur(a, z, w, r, lam, delta, alpha, upsilon)
...
def compute_aggregate_moments(dist, a_grid, z_grid, policy_occ, w, r, lam, delta, alpha, upsilon):
    ...
    A = np.sum(dist * a_grid[:, np.newaxis])
...
def find_equilibrium(a_grid, z_grid, prob_z, params,
    ...
    for iteration in range(max_iter):
        ...
        if total_error < best_error:
            ...
            best_result = {'w': w, 'r': r, 'agg': agg.copy(),
                          'dist': dist.copy(), 'policy_occ': policy_occ.copy()}
...
if __name__ == "__main__":
    ...
    _ = solve_entrepreneur(1.0, 1.5, 1.0, 0.04, 2.0, DELTA, ALPHA, NU)
    params_test = (DELTA, ALPHA, NU, 2.0, BETA, SIGMA, PSI)
    V_test = np.zeros((10, 5))
    _ = bellman_operator(V_test, a_grid[:10], z_grid[:5], prob_z[:5], 1.0, 0.04,
                        DELTA, ALPHA, NU, 2.0, BETA, SIGMA, PSI)
    ...
    plt.tight_layout()
    plt.savefig('/home/nuagsire/Dropbox/PhD Bocconi/Replications/BueraShin/figure2_replication_v2.png',
                dpi=150, bbox_inches='tight')
    plt.close()
    print("\nFigure 2 saved to 'figure2_replication_v2.png'")
    ...
    plt.tight_layout()
    plt.savefig('/home/nuagsire/Dropbox/PhD Bocconi/Replications/BueraShin/figure2_diagnostics_v2.png',
                dpi=150, bbox_inches='tight')
    plt.close()
    print("Diagnostic plots saved to 'figure2_diagnostics_v2.png'")
