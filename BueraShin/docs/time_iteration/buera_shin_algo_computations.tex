...
\end{equation}
Defining marginal utility as $MU(a, z) = (y(a, z) - a'(a, z))^{-\sigma}$, the equation becomes:
\begin{equation}
...
\subsection{Detailed First-Order Condition in Spectral Form}
Let $\bm{\Phi}(a, z)$ be the vector of bivariate Chebyshev basis functions, such that the savings policy is $\hat{a}(a, z; \bm{C}) = \bm{\Phi}(a, z)^\top \bm{C}$. The household's optimal choice $a'$ satisfies the following first-order condition:
\begin{equation}
    F(a', a, z; \bm{C}) \equiv [y(a, z) - a']^{-\sigma} - \beta(1+r) \left[ \psi \text{MU}' + (1-\psi) \mathbb{E}_{z'} \text{MU}' \right] = 0
\end{equation}
where the next-period marginal utility $\text{MU}'$ is evaluated using the \textbf{known} coefficients $\bm{C}$ from the previous iteration:
\begin{equation}
    \text{MU}'(a', z'; \bm{C}) = \left[ y(a', z') - \bm{\Phi}(a', z')^\top \bm{C} \right]^{-\sigma}
\end{equation}
In this formulation, $a'$ is the target savings level for the current period (the variable we solve for), while $\bm{C}$ defines the behavior of the agent from $t+1$ onwards. The spectral method is powerful because $\bm{\Phi}(a', z')$ and its derivatives are defined for any $a' \in [a_{min}, a_{max}]$, allowing us to solve $F(a')=0$ using continuous root-finding methods (e.g., Brent's method) or by direct inversion.
...
\begin{enumerate}
    ...
    \item \textbf{Compute New Policy}: The target savings is $a^*_{target} = y(a_i, z_j) - c^*$. We apply the constraint $a' = \max\{a_{min}, a^*_{target}\}$.
