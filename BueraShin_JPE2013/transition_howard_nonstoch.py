...
@njit(cache=False)
def step_distribution_nodist(mu_in, pol_idx, a_grid, psi, prob_z):
    """
    Forward induction: mu_{t+1}(a', z') given mu_t(a, z) and policy pol_idx(a, z).
    Logic:
      1. For each (a, z), the mass mu[i, j] goes to a' = a_grid[pol_idx[i, j]].
      2. Since it's on grid, no splitting needed if using pol_idx directly, 
         BUT let's keep it robust for level-based policies if needed.
         Actually, we use discrete asset choices here (pol_idx).
      3. Ability transition: 
         - with prob psi: z' = z
         - with prob 1-psi: z' ~ prob_z
    """
    n_a, n_z = mu_in.shape
    mu_out = np.zeros_like(mu_in)
    ...
    for ia in range(n_a):
        for iz in range(n_z):
            ...
            for izp in range(n_z):
                mu_out[iap, izp] += m_reset * prob_z[izp]
                
    return mu_out

@njit(cache=False)
def step_distribution_dist(mup_in, mum_in, polp_idx, polm_idx, a_grid, psi, prob_z, prob_tau_p):
    """
    Pre-reform forward induction with two types.
    tau persists with ability. If reset, new tau draw depends on new z.
    """
    n_a, n_z = mup_in.shape
    mup_out = np.zeros_like(mup_in)
    mum_out = np.zeros_like(mum_in)
    ...
    for ia in range(n_a):
        for iz in range(n_z):
            ...
            if m > 1e-18:
                iap = polp_idx[ia, iz]
                # Stay
                mup_out[iap, iz] += m * psi
                # Reset
                m_reset = m * (1.0 - psi)
                for izp in range(n_z):
                    ptp = prob_tau_p[izp]
                    mup_out[iap, izp] += m_reset * ptp * prob_z[izp]
                    mum_out[iap, izp] += m_reset * (1.0 - ptp) * prob_z[izp]
...
def steady_state_post(a_grid, z_grid, prob_z, N, burn, seed=42, verbose=True):
    ...
    # Initialize distribution mu(a,z)
    # Start with a simple uniform distribution or mass at low assets
    n_a, n_z = len(a_grid), len(z_grid)
    mu = np.zeros((n_a, n_z), dtype=np.float64)
    mu[0, :] = prob_z / n_z # Start everyone at low assets, distributed by ability
    mu /= mu.sum()
    ...
    for outer in range(60):
        ...
        for inner in range(50):
            ...
            # Iterate on distribution until stationary
            for _it_mu in range(1000):
                mu_new = step_distribution_nodist(mu, pol, a_grid, PSI, prob_z)
                diff_mu = np.max(np.abs(mu_new - mu))
                mu = mu_new
                if diff_mu < 1e-10:
                    break

            # build aggregate A = E[a]
            A = 0.0
            for ia in range(n_a):
                A += a_grid[ia] * mu[ia, :].sum()
...
def steady_state_pre(a_grid, z_grid, prob_z, prob_tau_plus, N, burn, seed=77, verbose=True):
    ...
    n_a, n_z = len(a_grid), len(z_grid)
    mu_p = np.zeros((n_a, n_z), dtype=np.float64)
    mu_m = np.zeros((n_a, n_z), dtype=np.float64)
    
    # Initial draw: everyone at min assets, tau according to prob_tau_plus
    for iz in range(n_z):
        ptp = prob_tau_plus[iz]
        mu_p[0, iz] = ptp * prob_z[iz]
        mu_m[0, iz] = (1.0 - ptp) * prob_z[iz]
    ...
    for outer in range(80):
        ...
        for inner in range(60):
            ...
            # Iterate on distribution until stationary
            for _it_mu in range(1000):
                mu_p_new, mu_m_new = step_distribution_dist(mu_p, mu_m, polp, polm, a_grid, PSI, prob_z, prob_tau_plus)
                diff_mu = max(np.max(np.abs(mu_p_new - mu_p)), np.max(np.abs(mu_m_new - mu_m)))
                mu_p, mu_m = mu_p_new, mu_m_new
                if diff_mu < 1e-10:
                    break
            ...
            A = 0.0
            for ia in range(n_a):
                A += a_grid[ia] * (mu_p[ia, :].sum() + mu_m[ia, :].sum())
...
# =============================================================================
def solve_transition_B2(pre_eq, post_eq, a_grid, z_grid, prob_z, T, verbose=True, lam=None):
    """
    ...
    # Initial distribution at t=0 comes from pre-reform steady state
    mu0 = (pre_eq["mu_p"] + pre_eq["mu_m"]).astype(np.float64, copy=True)
    ...
    for outer in range(MAX_OUTER):
        ...
        for inner in range(MAX_W_INNER):
            ...
            for tt in range(T):
                ...
                At = 0.0
                for ia in range(n_a):
                    At += a_grid[ia] * mu_curr[ia, :].sum()
                A_path[tt] = At
                
                # step forward deterministically
                mu_curr = step_distribution_nodist(mu_curr, pol_idx_path[tt], a_grid, PSI, prob_z)
    ...
    # =========================
    # Final evaluation: recompute policies & propagate mu once more using final sequences
    # =========================
    pol_idx_path = np.empty((T, n_a, n_z), dtype=np.int32)
    V_next = V_T.copy()
    for tt in range(T-1, -1, -1):
