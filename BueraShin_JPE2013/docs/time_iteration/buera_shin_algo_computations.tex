...
\begin{equation}
    V(a, z) = \max_{a'} \left\{ \frac{(y(a, z) - a')^{1-\sigma}-1}{1-\sigma} + \beta \mathbb{E}[V(a', z') | z] \right\}
\end{equation}
where the expected continuation value is:
\begin{equation}
    \mathbb{E}[V(a', z') | z] = \psi V(a', z) + (1-\psi) \int V(a', z') \pi(z') dz'
\end{equation}
...
\begin{equation}
    F(a'; a_i, z_j, \bm{\gamma}^{(n)}) \equiv \underbrace{(y_i - a')^{-\sigma}}_{\text{Current MU}} - \beta(1+r) \underbrace{\mathcal{E}(a'; \bm{\gamma}^{(n)})}_{\text{Expected Future MU}}
\end{equation}
